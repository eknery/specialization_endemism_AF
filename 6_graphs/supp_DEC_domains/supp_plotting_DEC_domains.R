...
# species presence into domains
spp_presence_domain = read.table("1_geographic_classification/spp_presence_domain.csv",sep=",", h=T)
...
###### visual parameters
# tip domains
tip_domains = as.matrix(spp_presence_domain[,-c(1,2)])
rownames(tip_domains) = spp_presence_domain$species

for (j in 1:ncol(tip_domains)){
  true_index = which(tip_domains[,j] == 1)
  tip_domains[true_index,j] = j
}
...
tiff("6_graphs/supp_DEC_domains/DEC_tree.tiff", units="in", width=5, height=8, res=600)
  phylo.heatmap(mcc,X=tip_domains, fsize=0.5, colors=c("#FFFFFF",domain_colors[1:8]),
                standardize=F, legend=F, labels=F, split=c(0.85,0.15))
  nodelabels(node=(1+n_tips):(n_tips+n_inner_nodes), pie= inner_node_probs, piecol=domain_colors, cex=1)
  axisPhylo(pos=c(1.1), font=2, cex.axis=0.5)
dev.off()